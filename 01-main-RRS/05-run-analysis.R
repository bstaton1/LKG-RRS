...
out_dir_parent = "01-main-RRS/model-output"
...
source("01-main-RRS/01-dredge.R")

# select the best model
cat("\nSelecting the Best Model\n")
source("01-main-RRS/02-find-best-model.R")

# obtain bootstrapped predictions from model
cat("\nPerforming Parametric Bootstrap\n")
source("01-main-RRS/03-bootstrap-best-model.R")

# determine if output for both progeny types now exists
both_analyses_done = all(c("total_juv", "adult") %in% basename(list.dirs(out_dir_parent)))
...
if (make_figures & both_analyses_done) {
  ...
  fig_dir = file.path(out_dir_parent, "..", "figures")
  if (!dir.exists(fig_dir)) dir.create(fig_dir)

  cat("\nProducing Final Figures\n")
  source("01-main-RRS/04-plots.R")
}
